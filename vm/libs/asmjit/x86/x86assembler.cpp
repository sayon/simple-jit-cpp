  ...
  if (op->isReg()) {
    ...
    switch (m->getMemType()) {
      ...
      case kMemTypeStackIndex:
        // [BaseTransform + index << shift + displacement]
...
static Error ASMJIT_CDECL X86Assembler_emit(Assembler* self_, uint32_t code, const Operand* o0, const Operand* o1, const Operand* o2, const Operand* o3) {
  ...
  // Memory operand BaseTransform register index.
  uint32_t mBase;
...
_EmitJmpOrCallAbs:
  {
    ...
    if (Arch == kArchX64) {
      ...
      // If the BaseTransform address of the output is known, it's possible to determine
