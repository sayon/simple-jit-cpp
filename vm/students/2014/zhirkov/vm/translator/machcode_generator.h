...
#define RegOrMemMatch(rm, ifmem, ifgp, ifxmm) switch (rm.type) {\
    case RegOrMem::RM_GPREG: { X86GpReg  const& gpReg = rm.gp; (void)gpReg; ifgp ; break;}\
    case RegOrMem::RM_XMMREG: { X86XmmReg const& xmmReg = rm.xmm; (void)xmmReg; ifxmm ; break;}\
    case RegOrMem::RM_MEM: { X86Mem const& mem = rm.mem;(void)mem; ifmem ; break;}\
...
#define RegOrMemMatchNoRedef(rm, ifmem, ifgp, ifxmm) switch ((rm).type) {\
    ...
    public:
        ...
        static void refToLocation(Reference refLoc, RegOrMem dest, X86Assembler& _);
