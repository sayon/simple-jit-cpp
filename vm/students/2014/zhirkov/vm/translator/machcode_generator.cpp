...
namespace mathvm {
    ...
    void CodeGenerator::prologue(CodeGenerator::Function const &f) {
        ...
        auto subsize = (locals + f.backedXmmArgsRegsRbpOffsets.size()) * 8;
        if (subsize)
            _.sub(rsp, subsize) ;
        for(size_t i = 0; i < f.backedXmmArgsRegsRbpOffsets.size(); i++) _.movq(ptr(rbp, -locals*8 - 8 *i), sseArgumentsRegs[i]);
    }
    ...
    bool CodeGenerator::hasStackFrame(IR::Function const &f) const {
        ...
        return locals > 0 || _functions[f.id]->hasMemArgs()
                || hasCalls(f) || f.id == 0   ||_functions[f.id]->backedXmmArgsRegsRbpOffsets.size() >0;
    }
...
//        _debug << "visiting block " << expr->name << "\n";
        ...
        for (auto st:expr->contents) {
            ...
            std::stringstream str; str<< ";>>" << (*st).num << " : " << *st << "\n";

            if (_.getLogger()) _.getLogger()->logFormat(0, str.str().c_str());
    ...
    void CodeGenerator::visit(IR::ReadRef const *const expr) {
        ...
        refToLocation(refLoc, _nextAssignment, _);
    }

    void CodeGenerator::refToLocation(Reference refLoc, RegOrMem dest, X86Assembler& _) {
        if (refLoc.type == Reference::REF_RM) {
            ...
            RegOrMemMatch(dest, {_.mov(rax, refLoc.asMemCell()); _.mov(mem, rax);}, _.mov(gpReg, refLoc.asMemCell());, _.movq(xmmReg, refLoc.asMemCell()); )
        }
    ...
    void CodeGenerator::visit(const IR::WriteRef *const expr) {
        ...
        RegOrMemMatch(refLoc.rm, {
            if (sse) _nextAssignment = RegOrMem(tempXmmRegs[0]); else _nextAssignment = RegOrMem(tempGpRegs[0]);
            expr->atom->visit(this);
            if (saveRbx) _.push(rbx);
            _.mov(rbx, mem);
            if (sse) _.movq(ptr(rbx), tempXmmRegs[0]); else _.mov(ptr(rbx), tempGpRegs[0]);
...
//        }
    ...
    RegOrMem CodeGenerator::locate(IR::VarId id) const {
        if (id == gpAccVar) return RegOrMem(tempGpRegs[0]);
        if (id == doubleAccVar) return RegOrMem(tempXmmRegs[0]);
        ...
        if (arg)
            switch (arg->class_) {
                ...
                case AC_SSE:
                    for(size_t i = 0; i < f->backedXmmArgsRegsRbpOffsets.size(); i++)
                        if (arg->xmmReg == sseArgumentsRegs[i]) return RegOrMem(X86Mem(rbp, f->backedXmmArgsRegsRbpOffsets[i]));
            ...
            if (vreg == gpAccVreg)
                return RegOrMem(tempGpRegs[0]);
            if (vreg == doubleAccVreg)
                return RegOrMem(tempXmmRegs[0]);
