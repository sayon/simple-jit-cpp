...
namespace mathvm {
    ...
    void SimpleIrBuilder::visitIfNode(IfNode *node) {
        ...
        auto condVar = makeTempVar();
        emit(new IR::Assignment(condVar, _popAtom()));

        IR::Block *afterIf = newBlock(std::string("if_var") + to_string(condVar) + "_end");
        IR::Block *yesblock = newBlock(std::string("if_var") + toString(condVar) + "_true");
        IR::Block *noblock = newBlock(std::string("if_var") + toString(condVar) + "_false");

        if (node->elseBlock())
            ctx.block->link(new IR::JumpCond(yesblock, noblock, new IR::Variable(condVar)));
        else
            ctx.block->link(new IR::JumpCond(yesblock, afterIf, new IR::Variable(condVar)));


        ctx.block = yesblock;
        node->thenBlock()->visit(this);
        ctx.block->link(afterIf);
