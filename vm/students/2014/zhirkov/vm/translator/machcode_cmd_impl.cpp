...
#define RefMatch(operand, ifloc, ifmem, ifgp) if (operand.type == Reference::REF_RM) {\
    auto mem = operand.asMemCell(); (void)mem; ifloc;}\
else RegOrMemMatch(operand.rm, ifmem, ifgp, {throw CodeGenerationError("Pointer can't be stored in xmm register!");} );


namespace mathvm {


    static void genADD1 (X86Mem const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        RefMatch(operand,
                _.mov(rax, mem);
                _.add(dest, rax);,
    ...
    static void genADD1 (X86GpReg const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        RefMatch(operand,
                _.add(dest, mem);,
    ...
    static void genSUB1 (X86Mem const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        RefMatch(operand,
                _.mov(rax, mem);
                _.add(dest, rax);,
    ...
    static void genSUB1 (X86GpReg const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        RefMatch(operand,
                _.sub(dest, mem);,
    ...
    static void genDIV1 (X86Mem const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        RefMatch(operand,
                _.mov(rax, dest);
                _.push(rdx);
                _.cqo();
    ...
    static void genDIV1 (X86GpReg const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        RefMatch(operand,
                _.mov(rax, mem);
                _.push(rdx);
                _.cqo();
    ...
    static void genMUL1 (X86Mem const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        RefMatch(operand,
                _.mov(rax, dest);
                _.imul(rax, mem);
                _.mov(dest, rax);,
    ...
    static void genMUL1 (X86GpReg const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        RefMatch(operand,
                _.imul(dest, mem);,
                ...
                _.imul(dest, ptr(gpReg));
                )
    }


    static void genFADD1 (X86Mem const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        RefMatch(operand,
                ...
                _.mov(rax, mem);
                _.movq(tempXmmRegs[0], ptr(rax));
                _.addsd(tempXmmRegs[0], dest);
                _.movq(dest, tempXmmRegs[0]);,

                _.movq(tempXmmRegs[0], ptr(gpReg));
                _.addsd(tempXmmRegs[0], dest);
                _.movq(dest, tempXmmRegs[0]);
                )
    }
    ...
    static void genFADD1 (X86XmmReg const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        RefMatch(operand,
                _.addsd(dest, mem);,

                _.mov(rax, mem);
                _.addsd(dest, ptr(rax));,

                _.addsd(dest, ptr(gpReg));
                )
    }


    static void genFSUB1 (X86Mem const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        RefMatch(operand,
                _.movq(tempXmmRegs[0], dest);
                _.subsd(tempXmmRegs[0], dest);
                _.movq(dest, tempXmmRegs[0]);,
    ...
    static void genFSUB1 (X86XmmReg const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        RefMatch(operand,
                _.subsd(dest, mem);,

                _.mov(rax, mem);
                _.subsd(dest, ptr(rax));,

                _.subsd(dest, ptr(gpReg));
                )
    }


    static void genFMUL1 (X86Mem const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        RefMatch(operand,
                _.movq(tempXmmRegs[0], dest);
    ...
    static void genFMUL1 (X86XmmReg const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        RefMatch(operand,
                _.mulsd(dest, mem);,

                _.mov(rax, mem);
                _.mulsd(dest, ptr(rax));,

                _.mulsd(dest, ptr(gpReg));
                )
    }


    static void genFDIV1 (X86Mem const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        RefMatch(operand,
                _.movq(tempXmmRegs[0], dest);
                _.divsd(tempXmmRegs[0], dest);
                _.movq(dest, tempXmmRegs[0]);,
    ...
    static void genFDIV1 (X86XmmReg const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        RefMatch(operand,
                _.divsd(dest, mem);,

                _.mov(rax, mem);
                _.divsd(dest, ptr(rax));,

                _.divsd(dest, ptr(gpReg));
                )
    ...
    static void genMOD1 (X86Mem const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        RefMatch(operand,
                _.mov(rax, dest);
                _.push(rdx);
                _.cqo();
    ...
    static void genMOD1 (X86GpReg const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        RefMatch(operand,
                _.mov(rax, mem);
                _.push(rdx);
                _.cqo();
...
//    static void genLT1 (X86Mem const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
//        RefMatch(operand,
//                _.mov(rax, mem);
//                _.cmp(rax, dest);
//                _.setg(al);
...
//    static void genLT1(X86GpReg const &dest, Reference operand, X86Assembler &_, MvmRuntime &runtime) {
//        RefMatch(operand,
//                _.cmp(dest, mem);
//                _.setl(al);
//                _.movzx(dest, al);,
...
//    static void genFLT1 (X86Mem const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
//        RefMatch(operand,
//                _.movq(tempXmmRegs[0], dest);
//                _.ucomisd(tempXmmRegs[0], mem);
//                _.setl(al);
//                _.movzx(rax, al);
//                _.mov(dest, rax);,
//
...
//    }
    ...
    static void genFLT2 (X86GpReg const& dest, Reference fst, X86XmmReg snd, X86Assembler& _, MvmRuntime& runtime) {
        RefMatch(fst,
                _.ucomisd(snd, mem);
                _.setg(al);
                _.movzx(dest, al);,
                ...
                _.mov(rax, mem);
                _.ucomisd(snd, ptr(rax));
                _.setg(al);
                _.movzx(dest, al);,

                _.ucomisd(snd, ptr(gpReg));
                _.setg(al);
                _.movzx(dest, al);
        )
    }

    static void genFLE2 (X86GpReg const& dest, Reference fst, X86XmmReg snd, X86Assembler& _, MvmRuntime& runtime) {
        RefMatch(fst,
                _.ucomisd(snd, mem);
                _.setge(al);
                _.movzx(dest, al);,
    ...
    static void genFLT2 (X86GpReg const& dest, Reference fst, X86Mem snd, X86Assembler& _, MvmRuntime& runtime) {
        _.movq(tempXmmRegs[0], snd);
        genFLT2(dest, fst, tempXmmRegs[0], _, runtime);
    }


    static void genFLE2 (X86GpReg const& dest, Reference fst, X86Mem snd, X86Assembler& _, MvmRuntime& runtime) {
        _.movq(tempXmmRegs[0], snd);
        genFLE2(dest, fst, tempXmmRegs[0], _, runtime);
    }

    static void genFLT2 (X86Mem const& dest, Reference fst, X86XmmReg snd, X86Assembler& _, MvmRuntime& runtime) {
        genFLT2(rax, fst, snd, _, runtime);
        _.mov(dest, rax);
    }


    static void genFLT2 (X86Mem const& dest, Reference fst, X86Mem snd, X86Assembler& _, MvmRuntime& runtime) {
        genFLT2(rax, fst, snd, _, runtime);
        _.mov(dest, rax);
    }

    static void genFLE2 (X86Mem const& dest, Reference fst, X86XmmReg snd, X86Assembler& _, MvmRuntime& runtime) {
        genFLE2(rax, fst, snd, _, runtime);
        _.mov(dest, rax);
    }


    static void genFLE2(X86Mem const& dest, Reference fst, X86Mem snd, X86Assembler& _, MvmRuntime& runtime) {
        genFLE2(rax, fst, snd, _, runtime);
        _.mov(dest, rax);
    }


    static void genFLT2 (X86GpReg const& dest, X86XmmReg fst, Reference snd, X86Assembler& _, MvmRuntime& runtime) {
        RefMatch(snd,
                _.ucomisd(fst, mem);
                _.setl(al);
                _.movzx(dest, al);,
                ...
                _.mov(rax, mem);
                _.ucomisd(fst, ptr(rax));
                _.setl(al);
                _.movzx(dest, al);,

                _.ucomisd(fst, ptr(gpReg));
                _.setl(al);
                _.movzx(dest, al);
        )
    }

    static void genFLE2 (X86GpReg const& dest, X86XmmReg fst, Reference snd, X86Assembler& _, MvmRuntime& runtime) {
        RefMatch(snd,
                _.ucomisd(fst, mem);
                _.setle(al);
                _.movzx(dest, al);,
    ...
    static void genFLT2 (X86GpReg const& dest, X86Mem fst, Reference snd, X86Assembler& _, MvmRuntime& runtime) {
        _.movq(tempXmmRegs[0], fst);
        genFLT2(dest, tempXmmRegs[0], snd, _, runtime);
    }


    static void genFLE2 (X86GpReg const& dest, X86Mem fst, Reference snd, X86Assembler& _, MvmRuntime& runtime) {
        _.movq(tempXmmRegs[0], fst);
        genFLE2(dest, tempXmmRegs[0], snd, _, runtime);
    }

    static void genFLT2 (X86Mem const& dest,  X86XmmReg fst, Reference snd, X86Assembler& _, MvmRuntime& runtime) {
        genFLT2(rax, fst, snd, _, runtime);
        _.mov(dest, rax);
    }


    static void genFLT2 (X86Mem const& dest, X86Mem fst, Reference snd, X86Assembler& _, MvmRuntime& runtime) {
        genFLT2(rax, fst, snd,  _, runtime);
        _.mov(dest, rax);
    }

    static void genFLE2 (X86Mem const& dest, X86XmmReg fst, Reference snd, X86Assembler& _, MvmRuntime& runtime) {
        genFLE2(rax, fst, snd, _, runtime);
        _.mov(dest, rax);
    }


    static void genFLE2(X86Mem const& dest, X86Mem fst, Reference snd, X86Assembler& _, MvmRuntime& runtime) {
        genFLE2(rax, fst, snd, _, runtime);
        _.mov(dest, rax);
    }
    ...
    static void genEQ1 (X86GpReg const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        CodeGenerator::refToLocation(operand, RegOrMem(rax), _);
        _.cmp(rax, dest);
        _.sete(al);
        _.movzx(dest, al);
    }

    static void genEQ1 (X86Mem const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        genEQ1(rax, operand, _, runtime);
        _.mov(dest, rax);
    }

    static void genNEQ1 (X86GpReg const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        CodeGenerator::refToLocation(operand, RegOrMem(rax), _);
        _.cmp(rax, dest);
        _.setne(al);
        _.movzx(dest, al);
    ...
    static void genNEQ1 (X86Mem const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        genNEQ1(rax, operand, _, runtime);
        _.mov(dest, rax);
    }


    static void genOR1 (X86GpReg const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {

    }


    static void genOR1 (X86Mem const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {

    }


    static void genAND1 (X86GpReg const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {

    }


    static void genAND1 (X86Mem const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {

    }


    static void genLOR1 (X86GpReg const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {

    }


    static void genLOR1 (X86Mem const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {

    }


    static void genLAND1 (X86GpReg const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {

    }


    static void genLAND1 (X86Mem const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {

    }


    static void genXOR1 (X86GpReg const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {

    }


    static void genXOR1 (X86Mem const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {

    }


    static void genCAST_I2D1 (X86XmmReg const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        RefMatch(operand,
                _.mov(rax, mem);
                _.cvtsi2sd(dest, rax);,
    ...
    static void genCAST_I2D1 (X86Mem const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        genCAST_I2D1(tempXmmRegs[0], operand, _, runtime);
        _.movq(dest, tempXmmRegs[0]);
    }


    static void genCAST_D2I1 (X86GpReg const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        CodeGenerator::refToLocation(operand, RegOrMem(tempXmmRegs[0]), _);
        _.cvtsd2si(dest, tempXmmRegs[0]);
    }


    static void genCAST_D2I1(X86Mem const &dest, Reference operand, X86Assembler &_, MvmRuntime &runtime) {
        genCAST_D2I1(rax, operand, _, runtime);
        _.mov(dest, rax);
    }


    static void genNEG1 (X86GpReg const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        CodeGenerator::refToLocation(operand, RegOrMem(dest), _);
        _.neg(dest);
    }


    static void genNEG1 (X86Mem const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        genNEG1(rax, operand, _, runtime);
        _.mov(dest, rax);
    }


    static void genNOT1 (X86GpReg const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        CodeGenerator::refToLocation(operand, RegOrMem(dest), _);
        _.test(dest, dest);
        _.setz(al);
        _.movzx(dest, al);
    ...
    static void genNOT1 (X86Mem const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        genNOT1(rax, operand, _, runtime);
        _.mov(dest, rax);
    }


    static void genFNEG1 (X86XmmReg const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        CodeGenerator::refToLocation(operand, tempXmmRegs[1], _);
        _.movq(tempXmmRegs[0], ptr_abs(Ptr(&MvmRuntime::FNEG_MASK)));
        _.xorps(tempXmmRegs[0],tempXmmRegs[1]);
        if (dest != tempXmmRegs[0]) _.movq(dest, tempXmmRegs[0]);
    }


    static void genFNEG1 (X86Mem const& dest, Reference operand, X86Assembler& _, MvmRuntime& runtime) {
        genFNEG1(tempXmmRegs[0], operand, _, runtime);
        _.movq(dest, tempXmmRegs[0]);
    }
    ...
    static void genFSUB2(X86Mem const &dest, X86Mem const &fst, double    snd, X86Assembler &_, MvmRuntime& runtime) {
        _.movq(tempXmmRegs[0], fst);
        _.movq(tempXmmRegs[1],ptr_abs(Ptr(runtime.vivify(snd))));
        _.subsd(tempXmmRegs[0], tempXmmRegs[1]);
        _.movq(dest, tempXmmRegs[0]);
    }
    ...
    static void genFLE2(X86GpReg const &dest, Reference fst, Reference snd, X86Assembler &_, MvmRuntime& runtime) {
        CodeGenerator::refToLocation(snd, RegOrMem(tempXmmRegs[1]),_);
        genFLE2(dest, fst, tempXmmRegs[1], _, runtime);
    }

    static void genFLT2(X86GpReg const &dest, Reference fst, Reference snd, X86Assembler &_, MvmRuntime& runtime) {
        CodeGenerator::refToLocation(snd, RegOrMem(tempXmmRegs[1]),_);
        genFLT2(dest, fst, tempXmmRegs[1], _, runtime);
    }

    static void genFLE2(X86Mem const &dest, Reference fst, Reference snd, X86Assembler &_, MvmRuntime& runtime) {
        CodeGenerator::refToLocation(snd, RegOrMem(tempXmmRegs[1]),_);
        genFLE2(dest, fst, tempXmmRegs[1], _, runtime);
    }

    static void genFLT2(X86Mem const &dest, Reference fst, Reference snd, X86Assembler &_, MvmRuntime& runtime) {
        CodeGenerator::refToLocation(snd, RegOrMem(tempXmmRegs[1]),_);
        genFLT2(dest, fst, tempXmmRegs[1], _, runtime);
    }
    ...
    static void genFNEG1(X86XmmReg const &dest, X86Mem const &operand, X86Assembler &_, MvmRuntime& runtime) {
        _.movq(tempXmmRegs[1], ptr_abs(Ptr(&MvmRuntime::FNEG_MASK)));
        _.movq(dest, operand);
        _.xorps(dest,tempXmmRegs[1]);
    }


    static void genFNEG1(X86Mem const &dest, X86XmmReg const &operand, X86Assembler &_, MvmRuntime& runtime) {
        _.movq(tempXmmRegs[0], ptr_abs(Ptr(&runtime.FNEG_MASK)));
        _.movq(tempXmmRegs[1], operand);
        _.xorps(tempXmmRegs[0],tempXmmRegs[1]);
        _.movq(dest, tempXmmRegs[0]);
    }
    ...
    void CodeGenerator::genBinOp(IR::BinOp::Type type, RegOrMem lhs, IR::Atom const *const fst, IR::Atom const *const snd, X86Assembler& _, MvmRuntime& runtime) {
        if (fst->isReadRef() && type != IR::BinOp::BO_FLT && type != IR::BinOp::BO_FLE)  {
            refToLocation(locateRef(_currentFunction->id, fst->asReadRef()->refId), lhs,_);
            genReducedBinOp(type, lhs, snd, _ , runtime);
            return;
        }


        if (fst-> isReadRef() && snd->isReadRef()) {
            if (type == IR::BinOp::BO_FLE && lhs.isGp() )  { genFLE2(lhs.gp,   locateRef(_currentFunction->id, fst->asReadRef()->refId), locateRef(_currentFunction->id, snd->asReadRef()->refId), _, runtime); return; }
            if (type == IR::BinOp::BO_FLE && lhs.isMem())  { genFLE2(lhs.mem,  locateRef(_currentFunction->id, fst->asReadRef()->refId), locateRef(_currentFunction->id, snd->asReadRef()->refId), _, runtime); return; }
            if (type == IR::BinOp::BO_FLT && lhs.isGp() )  { genFLT2(lhs.gp,   locateRef(_currentFunction->id, fst->asReadRef()->refId), locateRef(_currentFunction->id, snd->asReadRef()->refId), _, runtime); return; }
            if (type == IR::BinOp::BO_FLT && lhs.isMem())  { genFLT2(lhs.mem,  locateRef(_currentFunction->id, fst->asReadRef()->refId), locateRef(_currentFunction->id, snd->asReadRef()->refId), _, runtime); return; }
        }
        if (fst-> isReadRef()) {
            if (type == IR::BinOp::BO_FLE && lhs.isGp()  && snd->isVariable() && locate(snd->asVariable()).isXmm())  { genFLE2(lhs.gp, locateRef(_currentFunction->id, fst->asReadRef()->refId), locate(snd->asVariable()).xmm, _, runtime); return; }
            if (type == IR::BinOp::BO_FLE && lhs.isMem() && snd->isVariable() && locate(snd->asVariable()).isXmm())  { genFLE2(lhs.mem, locateRef(_currentFunction->id, fst->asReadRef()->refId), locate(snd->asVariable()).xmm, _, runtime); return; }
            if (type == IR::BinOp::BO_FLE && lhs.isGp()  && snd->isVariable() && locate(snd->asVariable()).isMem())  { genFLE2(lhs.gp, locateRef(_currentFunction->id, fst->asReadRef()->refId), locate(snd->asVariable()).mem, _, runtime); return; }
            if (type == IR::BinOp::BO_FLE && lhs.isMem() && snd->isVariable() && locate(snd->asVariable()).isMem())  { genFLE2(lhs.mem, locateRef(_currentFunction->id, fst->asReadRef()->refId), locate(snd->asVariable()).mem, _, runtime); return; }
            if (type == IR::BinOp::BO_FLT && lhs.isGp()  && snd->isVariable() && locate(snd->asVariable()).isXmm())  { genFLT2(lhs.gp, locateRef(_currentFunction->id, fst->asReadRef()->refId), locate(snd->asVariable()).xmm, _, runtime); return; }
            if (type == IR::BinOp::BO_FLT && lhs.isMem() && snd->isVariable() && locate(snd->asVariable()).isXmm())  { genFLT2(lhs.mem, locateRef(_currentFunction->id, fst->asReadRef()->refId), locate(snd->asVariable()).xmm, _, runtime); return; }
            if (type == IR::BinOp::BO_FLT && lhs.isGp()  && snd->isVariable() && locate(snd->asVariable()).isMem())  { genFLT2(lhs.gp, locateRef(_currentFunction->id, fst->asReadRef()->refId), locate(snd->asVariable()).mem, _, runtime); return; }
            if (type == IR::BinOp::BO_FLT && lhs.isMem() && snd->isVariable() && locate(snd->asVariable()).isMem())  { genFLT2(lhs.mem, locateRef(_currentFunction->id, fst->asReadRef()->refId), locate(snd->asVariable()).mem, _, runtime); return; }
        }
        if (snd->isReadRef()) {
            if (type == IR::BinOp::BO_FLE && lhs.isGp()  && fst->isVariable() && locate(fst->asVariable()).isXmm())  { genFLE2(lhs.gp,   locate(fst->asVariable()).xmm, locateRef(_currentFunction->id, snd->asReadRef()->refId), _, runtime); return; }
            if (type == IR::BinOp::BO_FLE && lhs.isMem() && fst->isVariable() && locate(fst->asVariable()).isXmm())  { genFLE2(lhs.mem,  locate(fst->asVariable()).xmm, locateRef(_currentFunction->id, snd->asReadRef()->refId), _, runtime); return; }
            if (type == IR::BinOp::BO_FLE && lhs.isGp()  && fst->isVariable() && locate(fst->asVariable()).isMem())  { genFLE2(lhs.gp,   locate(fst->asVariable()).mem, locateRef(_currentFunction->id, snd->asReadRef()->refId), _, runtime); return; }
            if (type == IR::BinOp::BO_FLE && lhs.isMem() && fst->isVariable() && locate(fst->asVariable()).isMem())  { genFLE2(lhs.mem,  locate(fst->asVariable()).mem, locateRef(_currentFunction->id, snd->asReadRef()->refId), _, runtime); return; }
            if (type == IR::BinOp::BO_FLT && lhs.isGp()  && fst->isVariable() && locate(fst->asVariable()).isXmm())  { genFLT2(lhs.gp,   locate(fst->asVariable()).xmm, locateRef(_currentFunction->id, snd->asReadRef()->refId), _, runtime); return; }
            if (type == IR::BinOp::BO_FLT && lhs.isMem() && fst->isVariable() && locate(fst->asVariable()).isXmm())  { genFLT2(lhs.mem,  locate(fst->asVariable()).xmm, locateRef(_currentFunction->id, snd->asReadRef()->refId), _, runtime); return; }
            if (type == IR::BinOp::BO_FLT && lhs.isGp()  && fst->isVariable() && locate(fst->asVariable()).isMem())  { genFLT2(lhs.gp,   locate(fst->asVariable()).mem, locateRef(_currentFunction->id, snd->asReadRef()->refId), _, runtime); return; }
            if (type == IR::BinOp::BO_FLT && lhs.isMem() && fst->isVariable() && locate(fst->asVariable()).isMem())  { genFLT2(lhs.mem,  locate(fst->asVariable()).mem, locateRef(_currentFunction->id, snd->asReadRef()->refId), _, runtime); return; }
        }
        ...
        throw  CodeGenerationError("Bad argument for binary operation generator");
    }
    ...
    void  CodeGenerator::genReducedBinOp(IR::BinOp::Type type, RegOrMem lhs, IR::Atom const *const operand, X86Assembler &_, MvmRuntime& runtime) {
        if (type == IR::BinOp::BO_XOR 	&& lhs.isGp() 	&&  operand->isReadRef()) { genXOR1 (lhs.gp, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_XOR 	&& lhs.isMem() 	&&  operand->isReadRef()) { genXOR1 (lhs.mem, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_LAND 	&& lhs.isGp() 	&&  operand->isReadRef()) { genLAND1 (lhs.gp, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_LAND 	&& lhs.isMem() 	&&  operand->isReadRef()) { genLAND1 (lhs.mem, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_LOR 	&& lhs.isGp() 	&&  operand->isReadRef()) { genLOR1 (lhs.gp, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_LOR 	&& lhs.isMem() 	&&  operand->isReadRef()) { genLOR1 (lhs.mem, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_AND 	&& lhs.isGp() 	&&  operand->isReadRef()) { genAND1 (lhs.gp, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_AND 	&& lhs.isMem() 	&&  operand->isReadRef()) { genAND1 (lhs.mem, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_OR 	&& lhs.isGp() 	&&  operand->isReadRef()) { genOR1 (lhs.gp, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_OR 	&& lhs.isMem() 	&&  operand->isReadRef()) { genOR1 (lhs.mem, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_NEQ 	&& lhs.isGp() 	&&  operand->isReadRef()) { genNEQ1 (lhs.gp, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_NEQ 	&& lhs.isMem() 	&&  operand->isReadRef()) { genNEQ1 (lhs.mem, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_EQ 	&& lhs.isGp() 	&&  operand->isReadRef()) { genEQ1 (lhs.gp, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_EQ 	&& lhs.isMem() 	&&  operand->isReadRef()) { genEQ1 (lhs.mem, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_MOD 	&& lhs.isGp() 	&&  operand->isReadRef()) { genMOD1 (lhs.gp, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_MOD 	&& lhs.isMem() 	&&  operand->isReadRef()) { genMOD1 (lhs.mem, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_FDIV 	&& lhs.isXmm() 	&&  operand->isReadRef()) { genFDIV1 (lhs.xmm, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_FDIV 	&& lhs.isMem() 	&&  operand->isReadRef()) { genFDIV1 (lhs.mem, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_FMUL 	&& lhs.isXmm() 	&&  operand->isReadRef()) { genFMUL1 (lhs.xmm, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_FMUL 	&& lhs.isMem() 	&&  operand->isReadRef()) { genFMUL1 (lhs.mem, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_FSUB 	&& lhs.isXmm() 	&&  operand->isReadRef()) { genFSUB1 (lhs.xmm, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_FSUB 	&& lhs.isMem() 	&&  operand->isReadRef()) { genFSUB1 (lhs.mem, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_FADD 	&& lhs.isXmm() 	&&  operand->isReadRef()) { genFADD1 (lhs.xmm, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_FADD 	&& lhs.isMem() 	&&  operand->isReadRef()) { genFADD1 (lhs.mem, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_DIV 	&& lhs.isGp() 	&&  operand->isReadRef()) { genDIV1 (lhs.gp, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_DIV 	&& lhs.isMem() 	&&  operand->isReadRef()) { genDIV1 (lhs.mem, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_MUL 	&& lhs.isGp() 	&&  operand->isReadRef()) { genMUL1 (lhs.gp, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_MUL 	&& lhs.isMem() 	&&  operand->isReadRef()) { genMUL1 (lhs.mem, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_SUB 	&& lhs.isGp() 	&&  operand->isReadRef()) { genSUB1 (lhs.gp, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_SUB 	&& lhs.isMem() 	&&  operand->isReadRef()) { genSUB1 (lhs.mem, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_ADD 	&& lhs.isGp() 	&&  operand->isReadRef()) { genADD1 (lhs.gp, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::BinOp::BO_ADD 	&& lhs.isMem() 	&&  operand->isReadRef()) { genADD1 (lhs.mem, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }
    ...
    void CodeGenerator::genUnOp(IR::UnOp::Type type, RegOrMem lhs, IR::Atom const *const operand, X86Assembler &_, MvmRuntime& runtime) {
        if (type == IR::UnOp::UO_FNEG   && lhs.isXmm()     &&  operand->isReadRef()) { genFNEG1 (lhs.xmm, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::UnOp::UO_FNEG   && lhs.isMem()     &&  operand->isReadRef()) { genFNEG1 (lhs.mem, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::UnOp::UO_NOT    && lhs.isGp()  &&  operand->isReadRef()) { genNOT1 (lhs.gp, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::UnOp::UO_NOT    && lhs.isMem()     &&  operand->isReadRef()) { genNOT1 (lhs.mem, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::UnOp::UO_NEG    && lhs.isGp()  &&  operand->isReadRef()) { genNEG1 (lhs.gp, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::UnOp::UO_NEG    && lhs.isMem()     &&  operand->isReadRef()) { genNEG1 (lhs.mem, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::UnOp::UO_CAST_D2I   && lhs.isGp()  &&  operand->isReadRef()) { genCAST_D2I1 (lhs.gp, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::UnOp::UO_CAST_D2I   && lhs.isMem()     &&  operand->isReadRef()) { genCAST_D2I1 (lhs.mem, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::UnOp::UO_CAST_I2D   && lhs.isXmm()  &&  operand->isReadRef()) { genCAST_I2D1 (lhs.xmm, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }

        if (type == IR::UnOp::UO_CAST_I2D   && lhs.isMem()     &&  operand->isReadRef()) { genCAST_I2D1 (lhs.mem, locateRef(_currentFunction->id, operand->asReadRef()->refId), _, runtime); return; }
